import { handleBrandsRoute } from "./routes/brands.js";
import { handleTokensRoute } from "./routes/tokens.js";
import { handleValidationRoute } from "./routes/validation.js";
import { handleResolveRoute } from "./routes/resolve.js";
import { initBrandSchema } from "./services/brand-service.js";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // 2. DB test endpoint — also initializes brand schema
    if (pathname === "/db/test" && method === "GET") return dbTest(env);

    // ─── Brand API routes (/api/brands/* + /api/resolve) ───────────────────
    // Token + validation sub-routes must come before bare brand CRUD routes.
    if (pathname.startsWith("/api/brands/")) {
      const tokenMatch = pathname.match(/^\/api\/brands\/([^/]+)\/tokens(\/.*)?$/);
      if (tokenMatch) {
        const brandId = tokenMatch[1];
        const subPath = tokenMatch[2] ?? '';
        return handleTokensRoute(request, env, brandId, subPath);
      }

      const validateMatch = pathname.match(/^\/api\/brands\/([^/]+)\/validate$/);
      if (validateMatch && request.method === "POST") {
        return handleValidationRoute(request, env, validateMatch[1]);
      }

      const idMatch = pathname.match(/^\/api\/brands\/([^/]+)$/);
      return handleBrandsRoute(request, env, idMatch ? idMatch[1] : null);
    }

    if (pathname === "/api/brands") {
      return handleBrandsRoute(request, env, null);
    }

    if (pathname === "/api/resolve") {
      return handleResolveRoute(request, env);
    }
...
async function dbTest(env: Env): Promise<Response> {
  ...
  // Also initialize brand schema
  await initBrandSchema(env);
  return json({ status: "ok", message: "items table ready; brand schema initialized" });
}

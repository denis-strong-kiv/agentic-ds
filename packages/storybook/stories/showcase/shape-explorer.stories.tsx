...
function ComponentShowcase({ preset }: { preset: ShapePreset }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {/* Button */}
      <div>
        <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-foreground-muted)', marginBottom: 6 }}>Button</p>
      ...
      {/* Input */}
      <div>
        <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-foreground-muted)', marginBottom: 6 }}>Input</p>
        <input
      ...
      {/* Badge */}
      <div>
        <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-foreground-muted)', marginBottom: 6 }}>Badge</p>
      ...
      {/* Card */}
      <div>
        <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-foreground-muted)', marginBottom: 6 }}>Card</p>
...
function ShapeFrameWithStyle({ preset, mode = 'light' }: { preset: ShapePreset; mode?: ColorMode }) {
  ...
  return (
    <div id={frameId} style={{ background: tokens['--color-background-default'], color: tokens['--color-foreground-default'], padding: '1.5rem', borderRadius: 8, border: '1px solid rgba(0,0,0,0.08)' }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--color-foreground-muted)', marginBottom: '1.5rem' }}>{SHAPE_CONFIGS[preset].description}</p>
          ...
          return (
            <div key={brandId} style={{ border: '1px solid rgba(0,0,0,0.1)', borderRadius: 8, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '1.25rem', background: tokens['--color-background-default'], color: tokens['--color-foreground-default'] }}>
                ...
                <p style={{ fontSize: 11, color: 'var(--color-foreground-muted)', marginBottom: '1rem' }}>Shape preset: <strong>{preset}</strong></p>
